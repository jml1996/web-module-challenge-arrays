...
function addFlavor(arr, newFlavorStr){
    arr.unshift(newFlavorStr);
    return arr;
}
...
function removeFlavorByName(arr, removeStr){
    // const arr = arr;
...
function filterByWord(arr, findStr){
    const out = [];
    for(let i = 0; i < arr.length; i++){
...
function getRandomFlavors(originalFlavors, newFlavors, seasonalFlavors, regionalFlavors){
    let master = originalFlavors.concat(newFlavors, seasonalFlavors, regionalFlavors);
    let randList = [];
    for(let i = 0; i < 31; i++){
        let randIndex = Math.floor(Math.random() * (master.length + 1));
        randList.push(master[randIndex]);
    }
    return randList;
}
